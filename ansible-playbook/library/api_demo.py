...
class DemoApi:
    ...
    def __init__(self, username, password, token, uri):
        self.uri = uri
        self.session = requests.session()
        self.connect(username, password, token)

    def connect(self, username, password, token):
        if token:
...
EXAMPLES = r'''
- name: Clear all set character and numbers
  api_demo:
    action: clear
  delegate_to: localhost

- name: Get number from set character
  api_demo:
    character: "A"
    action: get
  delegate_to: localhost

- name: Set number on a character
  api_demo:
    character: "A"
    number: 4
    action: set
